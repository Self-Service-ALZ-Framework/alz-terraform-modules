...
resource "time_sleep" "ai_foundry_wait" {
  ...
  triggers = {
    # Re-run the sleep whenever the account is created or updated (etag changes on every mutation)
    ai_foundry_etag = tostring(azapi_resource.ai_foundry.output.etag)
  }
